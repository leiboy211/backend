...
def _has_required_step_content(step: dict) -> bool:
    title = str(step.get("title") or "").strip()
    description = str(step.get("description") or step.get("reason") or "").strip()
    return bool(title and description)


def _learning_steps_are_usable(steps: list[dict], *, min_steps: int = 3) -> bool:
    if len(steps or []) < min_steps:
        return False
    usable_count = sum(1 for step in steps if isinstance(step, dict) and _has_required_step_content(step))
    return usable_count >= min_steps


def _project_paths_are_usable(projects: list[dict], repos: list[dict]) -> bool:
    if not projects:
        return False
    expected_names = {str(repo.get("name") or "").strip().lower() for repo in repos if str(repo.get("name") or "").strip()}
    project_names = {str(project.get("repo_name") or "").strip().lower() for project in projects if str(project.get("repo_name") or "").strip()}
    if expected_names and not expected_names.issubset(project_names):
        return False
    for project in projects:
        steps = project.get("steps") or []
        if not _learning_steps_are_usable(steps, min_steps=3):
            return False
    return True


def _prepare_rule_based_learning_fallback(
    repos: list[dict],
    detected_skills: list[str] | None,
    practice_dimensions: list[dict] | None,
...
def infer_learning_path(
    ...
    fallback_steps = _prepare_rule_based_learning_fallback(
        repos,
        detected_skills,
        practice_dimensions,
        personalization_key,
    ...
    try:
        steps = flan_t5.infer_learning_path(
            _flan_model(),
            repos,
...
def infer_project_learning_paths(
    ...
    try:
        projects = flan_t5.infer_project_learning_paths(
            _flan_model(),
            repos=repos,
            detected_skills=detected_skills,
            project_keywords=project_keywords,
        ...
        logger.warning("FLAN-T5 project learning-path generation failed, using rule-based fallback projects: %s", exc)
        projects = fallback_projects
    else:
        if not _project_paths_are_usable(projects, repos):
    ...
    if projects is not fallback_projects and llm_refiner.is_enabled():
        try:
            refined_projects = llm_refiner.generate_project_learning_paths(
                ...
                max_projects=len(projects) or 1,
    ...
    for project in projects:
        for step in project.get("steps") or []:
            step["ai_explanation"] = (
                f"Recommended from repository evidence and detected practice signals ({model_signal}) for this step."
            )
            evidence = list(step.get("evidence") or [])
            evidence.append(f"Model signal: {model_signal}")
